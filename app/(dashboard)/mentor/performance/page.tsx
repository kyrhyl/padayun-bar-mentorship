...
interface MentorPerformancePageProps {
  searchParams: Promise<{ range?: string; subject?: string; page?: string; limit?: string }>;
}

export default async function MentorPerformancePage({ searchParams }: MentorPerformancePageProps) {
  const session = await requireRole(["mentor", "admin"]);
  const params = await searchParams;

  const performance = await getMentorPerformanceListService(session.user.id, {
    range: params.range,
  ...
  return (
    <section className="space-y-6">
      <div className="space-y-1">
        ...
        <p className="text-sm text-slate-600">Track assigned mentees by submissions, scores, and pending reviews.</p>
      </div>
            ...
            {performance.items.map((item) => (
              <tr key={item.menteeId}>
                <td className="px-4 py-3">
                  <Link href={`/mentor/performance/${item.menteeId}?range=${performance.filters.range}${performance.filters.subject ? `&subject=${encodeURIComponent(performance.filters.subject)}` : ""}`} className="font-medium text-slate-900 underline">
                    {item.menteeName}
                  </Link>
