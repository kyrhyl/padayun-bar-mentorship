...
import { getMentorMenteePerformanceDetailService } from "@/services/performance.service";

interface MentorMenteePerformancePageProps {
  params: Promise<{ menteeId: string }>;
  searchParams: Promise<{ range?: string; subject?: string }>;
}

export default async function MentorMenteePerformancePage({
  params,
  searchParams,
}: MentorMenteePerformancePageProps) {
  ...
  let details;
  try {
    details = await getMentorMenteePerformanceDetailService(session.user.id, menteeId, {
      range: query.range,
  ...
  return (
    <section className="space-y-6">
      <div className="space-y-1">
        <Link href="/mentor/performance" className="text-sm text-slate-600 underline">Back to performance list</Link>
